...
paging_init (void)
{
  ...
  for (page = 0; page < init_ram_pages; page++)
    {
      uintptr_t paddr = page * PGSIZE; //physical address, start from 0
      char *vaddr = ptov (paddr);	//ptov는 해당 paddr의 맵핑되는 vaddr 반환
      ...
      if (pd[pde_idx] == 0)
        {
          ...
          pd[pde_idx] = pde_create (pt_); //새로 할당한 page table의 주소를 entry에 등록하겠다
        }

      pt_[pte_idx] = pte_create_kernel (vaddr, !in_kernel_text); //vaddress는 어케구한걸까?
    }
...
run_task (char **argv)
{
  ...
  printf ("Executing '%s':\n", task);
#ifdef USERPROG
  process_wait (process_execute (task)); //this is the way program runs
	/* the flow is like this.(jinu)
	first, execute process_execute(task) which create new thread
	second, execute process_wait which wait the executed child process until it dies
	*/
#else
  run_test (task);
#endif
