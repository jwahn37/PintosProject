...
struct inode 
  {
    ...
    struct inode_disk data;             /* Inode content. */
		...
		struct semaphore sema;		//semaphore to block write operation by jinu2
  };
...
// get block address 
/*
block_sector_t get_block_addr(struct file* file)
{
...
inode_open (block_sector_t sector)
{
  ...
  inode->removed = false;
	sema_init(&inode->sema, 1);//by jinu (PROJECT2_2 User program file system call)
                ...
                off_t offset) 
{
  ...
  if (inode->deny_write_cnt)
	...
	sema_down(&inode->sema); //by jinu (PROJECT2_2 User program file system call)
  while (size > 0) 
    {
	...
	sema_up(&inode->sema); //by jinu (PROJECT2_2 User program file system call)
  free (bounce);
