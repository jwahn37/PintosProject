...
consume_some_resources (void)
{
  ...
  for (fd = 0; fd < fdmax; fd++)
  {
	  if (open (test_name) == -1)
    { 
			//printf("multi oom open fail!!!!\n");
			 break;
		}
	}
//	printf("multi oop open succcccess!\n");
}
  ...
  for (i = 0; i < howmany; i++)
    {
      ...
      if (n > EXPECTED_DEPTH_TO_PASS/2)
        {
					printf("abnormal child : %d %d %d\n",n,EXPECTED_DEPTH_TO_PASS, EXPECTED_DEPTH_TO_PASS/2);
			//		printf("abnormal child : %d %d\n",n, EXPECTED_DEPTH_TP_PASS);
			printf("call spawn_child!\n");
          child_pid = spawn_child (n + 1, CRASH);
      printf("CRASH spawn_child val ; %d\n",child_pid);
			    if (child_pid != -1)
            {
              if (wait (child_pid) != -1)
      ...
      if (child_pid == -1)
      {
			printf("!!!!!!!!!!!!!maximum depth is reached!!!!!!!!!!!!!!!!1\n");
			 return n;
			}
  ...
  if (n == 0)
    {
      if (expected_depth < EXPECTED_DEPTH_TO_PASS)
        fail ("should have forked at least %d times. %d", EXPECTED_DEPTH_TO_PASS, expected_depth);
